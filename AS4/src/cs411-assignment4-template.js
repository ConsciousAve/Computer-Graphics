...
function McMultiplyPoint(point) {
  return [
    Mc[0] * point[0] + Mc[1] * point[1] + Mc[2] * point[2] + Mc[3] * point[3],
    Mc[4] * point[0] + Mc[5] * point[1] + Mc[6] * point[2] + Mc[7] * point[3],
    Mc[8] * point[0] + Mc[9] * point[1] + Mc [10] * point[2] + Mc[11] * point[3],
    Mc[12] * point[0] + Mc[13] * point[1] + Mc[14] * point[2] + Mc[15] * point[3]
  ];
}
function findPoint( x, x1) {
  return x[0] * x1[0] + x[1] * x1[1] + x[2] * x1[2] + x[3] * x1[3];
 }
function interpolate(pk_n1,pk,pk_p1,pk_p2)
{
  var xpoint = [pk_n1.x, pk.x,pk_p1.x, pk_p2.x];
  var ypoint = [pk_n1.y,pk.y, pk_p1.y,pk_p2.y];

  var xpointMc = McMultiplyPoint(xpoint);
  var ypointMc = McMultiplyPoint(ypoint);
  
  for(var u=0; u<=1; u+=uStep){
    var Ux = [Math.pow(u, 3), u * u, u, 1];
    var Uy = [Math.pow(u, 3), u * u, u, 1];
    
    var xpointNew = findPoint(xpointMc, Ux);
    var ypointNew = findPoint(ypointMc, Uy);
    
    intrPts.push(xpointNew);
    intrPts.push(ypointNew);
  }
}
...
function Pointprocs() {
  ...
  if (nCtrlPts > 3){ 
    ...
    var pk    = {x: ctrlPts[2 + i * 2], y: ctrlPts[3 + i * 2]}; // p_{k}
